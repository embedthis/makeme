...
module embedthis.doc {
    ...
    function parseArgs(): Array {
        ...
        for (argind = 1 ; argind < App.args.length; argind++) {
            ...
            case "--tags":
                ...
                blend(symbols, Path(App.args[argind]).readJSON())
                break
    ...
    function ref(name: String): String {
        ...
        if (!reserved[sym]) {
            if (sym != 'void)' && sym != 'const') {
                App.log.warn("Cannot find link \"" + sym + "\"" + " type " + parts)
            }
        }
        return name
    }
    ...
    function clean(str: String): String {
        ...
        str = str.replace(/<para>|<emphasis>|<title>|<type>|<\/para>|<\/emphasis>|<\/title>|<\/type>/g, "")
        str = str.replace(/<ref refid="([^"]*#[^"]*)"/g, '<a class="ref" href="$1"')
                    ...
                    if (n.name() == "simplesect") {
                        ...
                        for each (p in n.parameteritem) {
                            emit('    <tr><td class="param">' + p.parameternamelist.parametername + '</td><td>' +
                                cleanDot(p.parameterdescription.para) + '</td></tr>')
                        }
                        emit('    </table></dd></dl>')
        ...
        for each (m in def.sectiondef.memberdef) {
            ...
            if (m.@kind == "variable") {
                ...
                if (m.briefdescription != "" || m.detaileddescription != "") {
                    emit('<tr><td class="param">' + clean(m.type) + '</td><td>' + field + '</td><td>')
                    if (m.briefdescription != "") {
    ...
    function emitIndicies(xml: XML) {
         ...
         */
        emit('<a name="Functions"></a>\n\n<h2>Function Index</h2>')
        emit('  <table class="apiIndex" title="Function Index">')
    ...
    function saveTags() {
        ...
        for (s in symbols) {
            if (!symbols[s].toString().contains('#')) {
