...
function init(base, name) {
     ...
     */
    me.targets._Products_ = { enable: true, home: base, type: 'group', xgroup: true, name: 'Products' }
    me.targets._All_ =      { enable: true, home: base, type: 'build', xtarget: true, name: 'All' }
    me.targets._Prep_ =     { enable: true, home: base, type: 'build', xtarget: true, name: 'Prep', 
    ...
    me.targets._All_.depends = targets
    me.targets._Products_.depends = targets
    ...
    me.targets._Prep_['generate-xcode'] = code
}

function term() {
    delete me.targets._Products_
    delete me.targets._All_
    delete me.targets._Prep_
    delete me.xtargets
    delete me.xgroups
    delete me.xbinaries
    makeDirGlobals()
            ...
            if (obj && obj.type == 'obj') {
                ...
                for each (hdr in obj.depends) {
                    if (hdr is Path && hdr.extension == 'h') {
                        let path = Path(hdr).relativeTo(me.dir.src)
...
function groups(base: Path) {
    ...
    for each (target in me.targets) {
        ...
        output(groupFooter.expand({NAME: name, PATH: me.dir.src.relativeTo(base)}))
    }
