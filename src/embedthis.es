        ...
        if (package.prefixes.contains(pname)) {
            ...
            if (!me.generating) {
                if (me.target.name != 'uninstall') {
                    ...
                    if (prefix.exists) {
                        if (prefix.toString().contains(me.settings.product)) {
                            safeRemove(prefix)
                        }
                    }
                    ...
                    if (me.generating || !prefixes[pname].exists) {
                        if (prefixes[pname].contains(me.settings.product)) {
...
function setupPackagePrefixes(kind, package) {
    ...
    if (me.installing) {
        ...
        me.platform.vname = me.settings.product + '-' + me.settings.version
...
public function uninstallBinary() {
    ...
    let name = (me.platform.os == 'windows') ? me.settings.title : me.settings.product
    if (package) {
        checkRoot()
        if (!me.generating) {
...
function makeSimplePackage(package, prefixes, fmt) {
    ...
    let generic = me.dir.rel.join(me.settings.product + '-' + fmt + '.tgz')
    generic.remove()
    zname.link(generic)
...
function makeTarPackage(prefixes) {
    let base = [me.settings.product, me.settings.version, me.platform.dist, me.platform.os, me.platform.arch].join('-')
    ...
    let generic = me.dir.rel.join(me.settings.product + '-tar' + '.tgz')
    generic.remove()
    zname.link(generic)
}
...
/* DISABLED
    case 'linux':
        if (me.platform.dist == 'ubuntu') {
            ...
            trace('Info', 'Cannot package for ' + me.platform.dist + ' linux distribution')
        }
        break
 */
    case 'macosx':
...
function packageMacosx(prefixes) {
    ...
    let staging = prefixes.staging
    let s = me.settings
    let base = [s.product, s.version, me.platform.dist, me.platform.os, me.platform.arch].join('-')
    let name = me.dir.rel.join(base).joinExt('tar', true)
...
function packageFedora(prefixes) {
    ...
    let owndirs = RegExp(me.settings.product)
...
public function syncup(from: Path, to: Path) {
    ...
    for each (item in tar.list()) {
        let path = to.join(item.extensions.slice(1).join('/'))
...
public function apidoc(dox: Path, headers, title: String, tags) {
    ...
    run('ejs ' + me.dir.makeme.join('gendoc.es') + ' --bare ' + '--title \"' + 
        me.settings.product.toUpper() + ' - ' + title + ' Native API\" --out ' + name + 
...
public function checkUninstalled() {
    let result = []
    for each (prefix in me.prefixes) {
        if (!prefix.name.contains(me.settings.product)) {
            continue
        }
        if (prefix.exists) {
            result.push(prefix)
...
public function genProjects(extensions = '', profiles = ["default", "static"], platforms = null) 
{
    ...
    try {
        ...
        for each (name in platforms) {
            trace('Generate', me.settings.product + '-' + name.replace(/-.*/, '') + ' projects')
...
function fixlibs() {
    ...
    let ldconfigSwitch = (me.platform.os == 'freebsd') ? '-m' : '-n'
    let ldconfig = Cmd.locate('ldconfig')
    if (ldconfig) {
        Cmd.run(ldconfig + ' ' + ldconfigSwitch + ' /usr/local/lib/' + me.settings.product + '/modules')
    }
    if (me.platform.dist == 'fedora') {
        Cmd.run('chcon /usr/bin/chcon -t texrel_shlib_t ' + me.prefixes.vapp.join('bin').files('*.so').join(' '))
    }
}
